...
function LitzWire_ACRes_CalcFunk(StrandDiam,CoilTurns, CoilLength,CoilDiam,NumLayers=1,NumStrands=1,BundleDiam=StrandDiam)
    ...
    Rac_Straight = Rdc .* (1 .+ F .+ k .* (NumStrands .* StrandDiam ./ BundleDiam).^2 .* G)
    Rac_Coiled = Rdc .* (1 .+ F .+ k .* (NumStrands .* StrandDiam ./ BundleDiam).^2 .*G .+ u .*(NumStrands .* StrandDiam ./ Pitch).^2 .*G)
